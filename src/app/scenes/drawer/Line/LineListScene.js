...
export default class LineListScene extends Component {
  ...
  handleSelfInputSubmit = () => {
    ...
    this.props.store.setChatStatus(this.state.selfInputChatStatus);
  };
          ...
          >
            ...
            {!this.state.noData &&
              ...
              <List containerStyle={{ marginBottom: 20, marginTop: -2 }}>
                {convs.map((conv, key) =>
                  <Conversation conv={conv} key={key} />,
                )}
              </List>}
            {this.state.noData &&
              !this.state.isLoading &&
