...
class Prey {
  ...
  @computed get interests() {
    ...
    return Object.prototype.toString.call(this.prey.interests) ===
      "[object Object]"
      ? this.prey.interests
      : [];
  }
  ...
  @computed get languages() {
    return Object.prototype.toString.call(this.prey.lang) === "[object Object]"
      ? this.prey.lang
      : {};
  }
  ...
  @action setPreyListByKey(key) {
    this.firebase.database().ref("users/" + key).once("value").then(snap => {
  ...
  @action mq(sexOrientation) {
    ...
    query
      .orderByChild("sexOrientation")
      .equalTo(sexOrientation)
      .once("value", snap => {

        if(!snap.exists()) {
          console.log("No prey found");
          return;
        } else {
          console.log("Prey found: ", snap.val());
        }

        snap.forEach(childsnap => {
          this.preyList.push(childsnap.val());
          console.log("prey: ", childsnap.val());
        });
      })
      .then(() => {
  ...
  @action handleLike = () => {
    const r = this.firebase
      .database()
      .ref("preys/" + this.prey.uid + "/likes")
      .child(this.prey.uid);
    r.set({ time: Moment().unix() });
    Actions.chat({
      name: this.prey.displayName,
      uid: this.prey.uid,
      chatType: "visitor",
      birthday: this.prey.birthday,
  ...
  checkGeocode() {
    if (
      this.checkObject(this.prey.geocode) &&
      this.checkObject(this.user.geocode)
    ) {
      if (
        typeof this.prey.geocode.lat === "number" &&
        typeof this.prey.geocode.lng === "number"
      ) {
        return true;
      } else {
        return false;
